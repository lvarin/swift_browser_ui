...
const CSelect = class {
  constructor(hostRef) {
    ...
    this._blurred = false;
    this._optionItems = [];
    this._getListItem = (item, index$1) => {
      var _a;
      ...
      const a11y = {
        role: 'option',
        'aria-posinset': (index$1 + 1).toString(),
        'aria-setsize': this._items.length.toString(),
      };
      if (isActive) {
        a11y['aria-selected'] = 'true';
      }
      return (index.h("li", Object.assign({}, a11y, { id: itemId, ref: (el) => this._itemRefs.push({ value: item.value, ref: el }), class: classes, "data-value": item.name, onClick: (event) => this._select(event, item) }), this.hasOptionItems ? (index.h("slot", { name: `option-${index$1}` })) : (item.name)));
    };
  ...
  _renderInputElement() {
    var _a, _b;
    return (index.h("div", { class: "c-input-menu__input", onClick: () => this._showMenu() }, index.h("input", { "aria-controls": 'results_' + this._id, "aria-readonly": "true", "aria-haspopup": "listbox", id: this._inputId, ref: (el) => (this._inputElement = el), autocomplete: "off", class: "c-input__input", type: "text", value: (_a = this._getLabel()) !== null && _a !== void 0 ? _a : null, name: (_b = this.name) !== null && _b !== void 0 ? _b : null, readonly: "true" }), index.h("div", { ref: (el) => (this._selectionElement = el), class: "c-input-menu__selection" })));
  }
  _renderMenu(style) {
    return (index.h("div", { class: {
        'c-input-menu__item-wrapper': true,
        'c-input-menu__item-wrapper--shadow': this.shadow,
      } }, index.h("ul", { id: 'results_' + this._id, "aria-activedescendant": this.activeListItemId, "aria-expanded": this.menuVisible.toString(), style: style, title: this.label || this.placeholder, class: this.menuVisible
        ? 'c-input-menu__items'
  ...
  render() {
    ...
    return (index.h(index.Host, { ref: (el) => registerClickOutside(this, el, () => this._hideMenu()) }, index.h("div", { id: 'announce-' + this._id, class: "visuallyhidden", "aria-live": "polite", "aria-atomic": "true" }, this.statusText), index.h("c-input", { autofocus: this.autofocus, disabled: this.disabled, "hide-details": this.hideDetails, hint: this.hint, id: this.hostId, "input-id": this._inputId, label: this.label, name: this.name, placeholder: this.placeholder, required: this.required, shadow: this.shadow, valid: this.valid, validate: this.validate, "validate-on-blur": this.validateOnBlur, validation: this.validation, value: this.value, variant: "select" }, index.h("slot", { name: "pre", slot: "pre" }), index.h("div", { class: "c-input__content" }, this._renderInputElement(), this._renderMenu(itemsPerPageStyle), this._renderChevron(), index.h("slot", { onSlotchange: this._handleSlotChange })), index.h("slot", { name: "post", slot: "post" }))));
  }
...
CSelect._uniqueId = 0;
CSelect.style = cInputMenuCss;

exports.c_select = CSelect;

//# sourceMappingURL=c-select.cjs.entry.js.map
