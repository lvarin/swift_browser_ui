...
const CDataTable = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    this.paginate = index.createEvent(this, "paginate", 7);
  ...
  _sortData(data) {
    ...
    this._sortedData = sorted.map((row) => {
      const { _hiddenData } = row, _rowData = __rest(row, ["_hiddenData"]);
      const hiddenData = _hiddenData.reduce((items, item) => {
        var _a;
        items[item.id] =
          item.value.plainTextValue ||
            ((_a = item.value.formattedValue) === null || _a === void 0 ? void 0 : _a.trim()) ||
            item.value.value;
        return items;
      }, {});
      const rowData = Object.entries(_rowData).reduce((items, [key, item]) => {
        var _a;
        items[key] =
          item.plainTextValue ||
            ((_a = item.formattedValue) === null || _a === void 0 ? void 0 : _a.toString().trim()) ||
            item.value;
        return items;
      }, {});
      return Object.assign(Object.assign({}, rowData), hiddenData);
    });
    return !this.hideFooter
      ? sorted.slice((_b = (_a = this.pagination) === null || _a === void 0 ? void 0 : _a.startFrom) !== null && _b !== void 0 ? _b : 0, ((_d = (_c = this.pagination) === null || _c === void 0 ? void 0 : _c.endTo) !== null && _d !== void 0 ? _d : this.data.length) + 1)
      : sorted;
  }
  _handleScrollWidth() {
  ...
  _renderAdditioanlDataRow(rowIndex, rowData = {}) {
    return (this._hasHiddenData && (index.h("tr", { class: {
        'additional-data': true,
        active: this._activeRows.includes(rowIndex),
      } }, index.h("td", { colSpan: 100 }, index.h("div", null, index.h("ul", null, this._renderHiddenCells(rowIndex), this._renderHiddenHeaderChildren(rowIndex, rowData)))))));
  }
  _renderCellData(key, options, colIndex, rowIndex) {
    ...
    return !!Tag ? (index.h(Tag, Object.assign({}, params, { onClick: (event) => {
        var _a;
  ...
  _renderCellChildren(options, index$1, key, data) {
  ...
  _renderExpansionIndicator(rowIndex, rowData = {}) {
    return (this._hasHiddenData && (index.h("td", null, index.h("div", { tabindex: "0", onKeyUp: (event) => this._handleKeyUp(event, rowIndex, rowData) }, index.h("svg", { width: "22", height: "22", viewBox: "0 0 24 24" }, index.h("path", { d: mdi.mdiChevronDown }))))));
  }
  _renderHiddenCells(index$1) {
    return this._data[index$1]._hiddenData
      .filter((d) => !this._extraHeaders.find((h) => h.key === d.id))
      .map(({ id, key, value: options }) => {
      const index$1 = this._headers.findIndex((h) => h.key === id);
      return (index.h("li", null, !!key && (index.h("div", { class: "title" }, index.h("span", null, key, ":"), this._renderCellData(id, options, index$1, index$1))), !!options.children && (index.h("div", { class: "children" }, this._renderCellChildren(options, index$1, id, {})))));
    });
  }
  _renderHiddenHeaderChildren(index$1, rowData) {
    return this._data[index$1]._hiddenData.map(({ id, key }) => {
      var _a;
      const headerIndex = this.headers.findIndex((h) => h.key === id);
      return (!!((_a = this.headers[headerIndex]) === null || _a === void 0 ? void 0 : _a.children) && (index.h("li", null, index.h("div", { class: "children" }, this._renderCellChildren(this.headers[headerIndex], index$1, key, rowData)))));
    });
  }
  _renderLoaderRow() {
    return (index.h("tr", null, index.h("td", { class: "loader", colSpan: 100 }, this.loading && (index.h("div", { class: "c-data-table__loader" }, index.h("div", { class: "loading-bar" }))))));
  }
  _renderRows() {
    return (!!this._data.length &&
      Object.values(this._data).map((rowData, rowIndex) => {
        ...
        return (index.h(index.Fragment, null, index.h("tr", { class: {
            active: this._activeRows.includes(rowIndex),
  ...
  _renderSelectionCell(isSelected, selectionValue) {
    return (this.selectable && (index.h("td", null, index.h("div", { class: "selection" }, index.h("c-checkbox", { value: isSelected, "hide-details": true, onChangeValue: () => this._onSelection(selectionValue) })))));
  }
  _renderSortIndicator(key) {
    let iconPath = mdi.mdiSwapVertical;
    if (key === this.sortBy && this.sortDirection === 'asc') {
      iconPath = mdi.mdiArrowUpThin;
    }
    if (key === this.sortBy && this.sortDirection === 'desc') {
      iconPath = mdi.mdiArrowDownThin;
    }
    return (index.h("svg", { width: "24", height: "24", viewBox: "0 0 24 24", tabindex: 0 }, index.h("path", { d: iconPath })));
  }
  _renderStatusTextRow() {
    return (!this._data.length && (index.h("tr", null, index.h("td", { class: "info", colSpan: 100 }, index.h("div", null, this.loading ? this.loadingText : this.noDataText)))));
  }
  _renderTableBody() {
    return (index.h("tbody", null, this._renderLoaderRow(), this._renderStatusTextRow(), this._renderRows()));
  }
  _renderTableCell(key, options, colIndex, rowIndex, rowData = {}) {
    ...
    return (!!header &&
      !isHidden && (index.h("td", null, index.h("div", { "data-align": header === null || header === void 0 ? void 0 : header.align, "data-justify": header === null || header === void 0 ? void 0 : header.justify }, this._renderCellData(key, options, colIndex, rowIndex), !!options.children && (index.h("div", { class: "children" }, this._renderCellChildren(options, rowIndex, key, rowData))), !!((_a = this.headers.find((header) => header.key === key)) === null || _a === void 0 ? void 0 : _a.children) && (index.h("div", { class: "children" }, this._renderCellChildren(this.headers.find((header) => header.key === key), rowIndex, key, rowData)))))));
  }
  ...
  _renderTableFooter() {
    const footerStyles = this.horizontalScrolling
      ...
      : {};
    return (!!this.pagination && (index.h("tfoot", null, index.h("tr", { ref: (el) => (this._footerElement = el) }, index.h("td", { colSpan: 100 }, index.h("div", { class: "c-data-table__footer", style: footerStyles }, index.h("c-pagination", Object.assign({}, this.footerOptions, { value: this.pagination, simple: this._isPaginationSimple, onChangeValue: (e) => this._onPaginationChange(e) }))))))));
  }
  _renderTableHeader() {
    const indicatorRowStyles = this.horizontalScrolling
      ...
      : {};
    return (index.h("thead", { class: { sticky: this.stickyHeader } }, index.h("tr", null, this.selectable && (index.h("th", { class: "selection" }, !this.singleSelection && (index.h("div", { class: "selection--heading" }, index.h("c-checkbox", { value: this._hasSelectionsOnPage(), indeterminate: this._isIndeterminate, "hide-details": true, onClick: (event) => this._onHeadingSelection(event), onKeyUp: (event) => this._onHeadingSelection(event) }))))), this._hasHiddenData && index.h("th", { class: "indicator" }), !!this._headers.length &&
      this._headers.map((header) => {
        ...
        return (index.h("th", Object.assign({}, params), index.h("div", { class: { sortable: isSortable } }, header.value, isSortable && this._renderSortIndicator(header.key))));
      })), this.horizontalScrolling && (index.h("tr", { class: "c-data-table__header-indicators" }, index.h("th", { colSpan: 100 }, index.h("div", { style: indicatorRowStyles }, this.firstCellHidden && (index.h("c-icon-button", { class: "first", size: "small", outlined: true, onClick: () => this._scrollLeft() }, index.h("c-icon", { path: mdi.mdiChevronLeft }))), index.h("c-spacer", null), this.lastCellHidden && (index.h("c-icon-button", { class: "last", size: "small", outlined: true, onClick: () => this._scrollRight() }, index.h("c-icon", { path: mdi.mdiChevronRight })))))))));
  }
  _scrollLeft() {
  ...
  render() {
    ...
    return (index.h(index.Host, null, index.h("table", { class: tableClasses, ref: (el) => (this._tableElement = el) }, index.h("colgroup", null, [
      ...(this.selectable ? ['_selection'] : []),
      ...(this.hiddenHeaders.length ? ['_hidden'] : []),
      ...this._headerKeys,
    ].map((key) => (index.h("col", { class: this.sortBy === key && 'sorted-column' })))), this._renderTableHeader(), this._renderTableBody(), !this.hideFooter && this._renderTableFooter())));
  }
  get element() { return index.getElement(this); }
  static get watchers() { return {
    "hiddenHeaders": ["onHiddenHeaderChange"],
    "data": ["onDataChange"],
...
CDataTable.style = cDataTableCss;

exports.c_data_table = CDataTable;

//# sourceMappingURL=c-data-table.cjs.entry.js.map
