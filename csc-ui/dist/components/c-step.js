...
const CStep$1 = proxyCustomElement(class CStep extends HTMLElement {
  constructor() {
    super();
    this.__registerHost();
    this.__attachShadow();
  ...
  static get style() { return cStepCss; }
}, [1, "c-step", {
    "complete": [4],
    "current": [4]
  }]);
function defineCustomElement$1() {
  ...
  const components = ["c-step"];
  components.forEach(tagName => { switch (tagName) {
    case "c-step":
      if (!customElements.get(tagName)) {
        customElements.define(tagName, CStep$1);
      }
      break;
  } });
}
...
export { CStep, defineCustomElement };
