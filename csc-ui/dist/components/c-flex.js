...
const CFlex$1 = proxyCustomElement(class CFlex extends HTMLElement {
  constructor() {
    super();
    this.__registerHost();
    this.__attachShadow();
  ...
  static get style() { return cFlexCss; }
}, [1, "c-flex"]);
function defineCustomElement$1() {
  if (typeof customElements === "undefined") {
    return;
  }
  const components = ["c-flex"];
  components.forEach(tagName => { switch (tagName) {
    case "c-flex":
      if (!customElements.get(tagName)) {
        customElements.define(tagName, CFlex$1);
      }
      break;
  } });
}
...
export { CFlex, defineCustomElement };
